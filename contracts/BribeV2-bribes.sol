...
contract V2Bribe { 
    ...
    */
    ...
    function notifyRewardAmount(address token, uint amount) external lock {
        require(amount > 0, 'Bribe must be greater than 0');
        ...
        require(rewardRate[token] > 0, 'rewardRate too low');
...
contract V2BribeFactory {
    address public lastBribe;

    function createBribe() external returns (address) {
        lastBribe = address(new V2Bribe(msg.sender));
        return lastBribe;
    }
}