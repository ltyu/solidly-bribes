...
//     GaugeController constant GAUGE = GaugeController(0x2F50D538606Fa9EDD2B11E2446BEb18C9D5846bB); // Curve Gauge controller - controls liquidity gauges and the issuance of coins through the gauges
//     ve constant VE = ve(0x5f3b5DfEb7B28CDbD7FAba78963EE202a494e2A2); // veCRV token

//     mapping(address => mapping(address => uint)) _claims_per_gauge; // addressX : { addressY : uint }
//     mapping(address => mapping(address => uint)) _reward_per_gauge;

//     mapping(address => mapping(address => uint)) public reward_per_token;

//     mapping(address => mapping(address => uint)) public active_period; // { gauge addr : { reward_token addr: timestamp } }

//     mapping(address => mapping(address => mapping(address => uint))) public last_user_claim; // Date of the last time a user claimed a particular token for a particular gauge
...
//     // Updates the period to current block.timestamp
//     // only updates if a week has passed
//     function _update_period(address gauge, address reward_token) internal returns (uint) {
//         uint _period = active_period[gauge][reward_token]; // defaults to 0 in the beginning
...
//             active_period[gauge][reward_token] = _period; // defaults to block.timestamp
//         }
//         return _period;
//     }
...
//             uint _last_vote = GAUGE.last_user_vote(user, gauge); // Last time a user voted for a particular gauge
//             if (_last_vote < _period) {
//                 uint _slope = GAUGE.vote_user_slopes(user, gauge).slope;
//                 _amount = _slope * reward_per_token[gauge][reward_token] / PRECISION;
//                 if (_amount > 0) {
